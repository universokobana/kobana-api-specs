...
:::info[Atenção às versões da API]

:::

> Você está na documentação da **v1.0** da API. Esta versão possui endpoints para apenas alguns serviços. **Pagamentos, Transferências, Extrato, Recebimento por Pix** e mais recursos estão disponíveis apenas na **v2.0**. Para acessar a v2.0 utilize o menu no canto superior esquerdo.
...
Se deseja saber mais sobre retornos de erros e códigos de erros especificos acessar a página [Erros](/api/erros)
