...
- Use o `client_id` e `client_secret` que você obteve conosco durante o registro para redirecionar o usuário para a `Authorize URL`. Opcionalmente inclua o `scope` para acessar alguma [informação em específico](/api/overview/authentication/permissions).
