...
- Use o `client_id` e `client_secret` que você obteve conosco durante o registro para fazer a requisição para solicitar um token. Opcionalmente inclua o `scope` para acessar alguma [informação em específico](/api/overview/authentication/permissions).
